...
where not exists (select 1 from dbo.Roles where Id = NEWID())
GO
insert into Roles (Id,Name)
select 2, N'Admin'
where not exists (select 2 from dbo.Roles where Id = NEWID())
GO
insert into Roles (Id,Name)
select 3, N'CompanyAdmin'
where not exists (select 3 from dbo.Roles where Id = NEWID())
GO
insert into Roles (Id,Name)
select 4, N'Manager'
where not exists (select 4 from dbo.Roles where Id = NEWID())
GO
insert into Roles (Id,Name)
select 5, N'User'
where not exists (select 5 from dbo.Roles where Id = NEWID())
GO
insert into Roles (Id,Name)
select 6, N'Guest'
where not exists (select 6 from dbo.Roles where Id = NEWID())
GO
